...
```bash
cd /root/Arma-bot && git pull --ff-only origin main && { test -f servers.local.json || cp servers.example.json servers.local.json; } && sed -i '/^[[:space:]]*SERVERS_CONFIG[[:space:]]*=/d' .env && printf '\nSERVERS_CONFIG=servers.local.json\n' >> .env && systemctl restart reforger-timer
```
